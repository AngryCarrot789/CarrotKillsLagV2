...
public class AreaSafety {
    public static Location GetSafeYLocation(Location start, int startHeight) {
        ...
        try {
            for (double y = newLocation.getY(); y < 258; y += 2) {
                newLocation.setY(y);
    ...
    public static int GetBlocksBelow(Location location) {
        int count = 0;
        Location copy = new Location(location.getWorld(), location.getX(), location.getY(), location.getY());
        for (double i = copy.getY(); i > 0; i--) {
            copy.setY(i);
            if (!IsBlockAir(copy.getBlock())) {
                count++;
            }
            else {
                return count;
